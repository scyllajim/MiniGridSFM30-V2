...
PREFIX="${PREFIX:-stage13_holdout_killgen_loads_seed}"
...
python scripts/summarize_seed_group.py \
  ...
  --prefix "$PREFIX" \
  --seeds "${SEEDS[@]}" \
  --out-csv reports/stage13_holdout_killgen_loads_5seed.csv \
  --out-md reports/stage13_holdout_killgen_loads_5seed.md

echo "=== Stage13 multi-seed experiment complete ==="
